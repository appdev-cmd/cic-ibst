...
export function coTheTrinhDuyet(vaiTro: VaiTro): boolean {
  return vaiTro === 'quan-tri' || vaiTro === 'lanh-dao' || vaiTro === 'truong-don-vi' || vaiTro === 'chuyen-vien';
}
 ...
 *    Trưởng đơn vị KHÔNG có thẩm quyền tự quyết toán loại HĐ này.
...
export function coTheQuyetToan(vaiTro: VaiTro, capKy: CapKy | null | undefined): boolean {
  if (vaiTro === 'quan-tri' || vaiTro === 'lanh-dao') return true;
  if (vaiTro === 'truong-don-vi') return capKy === 'don-vi-ky';
  return false;
}
